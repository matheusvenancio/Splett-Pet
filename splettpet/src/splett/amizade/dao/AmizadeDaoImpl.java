...
public class AmizadeDaoImpl extends GenericDao<Amizade> implements AmizadeDao {
	...
	public List<Usuario> listAmigos(Usuario usuario) {
		...
		Query q = em
				.createQuery("Select ud from Amizade a inner join a.usuarioDestino ud where ud.id != :id and a.status = :status and a.usuarioOrigem.id = :id");
	...
	private List<Usuario> listarAmigosOrigem(Usuario usuario) {
		EntityManager em = emf.createEntityManager();
		Query q = em
				.createQuery("Select uo from Amizade a inner join a.usuarioOrigem uo where uo.id != :id and a.status = :status and a.usuarioDestino.id = :id");
		q.setParameter("id", usuario.getId());
		q.setParameter("status", Status.ACEITO);
	...
	public List<Amizade> listSolicitacoes(Usuario usuario) {
		EntityManager em = emf.createEntityManager();
		Query q = em
				.createQuery("Select a from Amizade a inner join a.usuarioOrigem uo where uo.id != :id and a.status = :status and a.usuarioDestino.id = :id");
		q.setParameter("id", usuario.getId());
	...
	public boolean isSolicitado(Usuario u1, Usuario u2) {
		...
		for (Amizade amizade : solicitacoesU1) {
			if (amizade.getUsuarioDestino().equals(u2)
					|| amizade.getUsuarioOrigem().equals(u2))
				return true;
		}
